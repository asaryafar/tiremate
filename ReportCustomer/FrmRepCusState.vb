...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(FrmRepCusState))
        ...
        Me.PanelTypeCus.Size = New System.Drawing.Size(430, 160)
        ...
        'BtnCancel
        '
        Me.BtnCancel.Location = New System.Drawing.Point(336, 384)
        ...
        'BtnPrint
        '
        Me.BtnPrint.Location = New System.Drawing.Point(176, 384)
        ...
        Me.BtnSelect.Location = New System.Drawing.Point(392, 8)
        ...
        'BtnShowFinance
        '
        Me.BtnShowFinance.Location = New System.Drawing.Point(16, 384)
        ...
        'BtnPriv
        '
        Me.BtnPriv.Location = New System.Drawing.Point(256, 384)
        ...
        'pb1
        '
        Me.pb1.Location = New System.Drawing.Point(16, 411)
        Me.pb1.Name = "pb1"
        Me.pb1.Size = New System.Drawing.Size(400, 14)
        Me.pb1.TabIndex = 16
        Me.pb1.Text = "Wait ..."
...
#End Region
    ...
    Private Sub BtnCus_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnCus.Click
        ...
        If DsFrmRepCusState1.customer.Count = 0 Then
            DACustomer.Fill(DsFrmRepCusState1.customer)
        End If
        f.Text = "  Select Customer"
        f.TypeOfForm = FrmSelect.KindForm.customer
        f.DSMAIN = DsFrmRepCusState1
        f.ShowDialog()
        BtnCus.Enabled = True
    End Sub
    ...
    Private Sub BtnType_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnType.Click
        BtnType.Enabled = False
        Dim f As New FrmSelect
        If DsFrmRepCusState1.type.Count = 0 Then
            DsFrmRepCusState1.type.Clear()
            DAType.Fill(DsFrmRepCusState1.type)
        End If
        f.Text = "  Select Type"
    ...
    Private Sub BtnClass_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnClass.Click
        BtnClass.Enabled = False
        Dim f As New FrmSelect
        If DsFrmRepCusState1.clas.Count = 0 Then
            DsFrmRepCusState1.clas.Clear()
            DAClass.Fill(DsFrmRepCusState1.clas)
        End If
        f.Text = "  Select Class"
    ...
    Sub AdjNegative()
        ...
        'DAGeneral.SelectCommand.CommandText = " SELECT     cust_trtab_main.cod_customer, cust_trtab_main.f_name + ' ' + cust_trtab_main.l_name AS name, cust_trtab_main.address, " & _
        '                              "  RTRIM(LTRIM(bas_zipcode.city)) + ' ' + RTRIM(LTRIM(bas_zipcode.state)) + ' ' + RTRIM(LTRIM(cust_trtab_main.zip)) AS phone_1, " & _
        '                                "  GL_account_adjustment_head.refrence_no AS RefNo, GL_account_adjustment_head.date_adjustment AS [date], " & _
        '                                "  CASE WHEN GL_account_adjustment_head.finance_charge_flag = 1 THEN 'Finance Charge' WHEN GL_account_adjustment_head.finance_charge_flag = " & _
        '                                " 0 THEN CASE WHEN GL_account_adjustment_head.Gl_receive_payment_Refrence_no IS NOT NULL " & _
        '                                " THEN 'EARLY PAYMENT DISCOUNT' ELSE 'Adjustment' END END AS Descript, - GL_account_adjustment_dtl.amount_adjustment AS payment,  " & _
        '                                " cust_company.complete_name, cust_trtab_main.cod_class, cust_trtab_main.cod_type " & st & _
        '                                " , dbo.ChangeDate(GL_account_adjustment_head.date_adjustment) AS OrderDate, LTRIM(RTRIM(cust_trtab_main.l_name)) " & _
        '                                " + ' ' + LTRIM(RTRIM(cust_trtab_main.m_name)) + ' ' + LTRIM(RTRIM(cust_trtab_main.f_name)) AS OrderName, DATEDIFF([day], " & _
        '                                "  GL_payment_deposit_Due_Date_For_AR.AR_due_date, GETDATE()) AS days, GL_payment_deposit_Due_Date_For_AR.AR_due_date " & _
        '                                " FROM         GL_account_adjustment_head INNER JOIN" & _
        '                                " cust_trtab_main ON GL_account_adjustment_head.cod_customer = cust_trtab_main.cod_customer INNER JOIN " & _
        '                                " GL_account_adjustment_dtl ON GL_account_adjustment_head.refrence_no = GL_account_adjustment_dtl.refrence_no INNER JOIN" & _
        '                                " GL_payment_deposit_Due_Date_For_AR ON  " & _
        '                                " GL_account_adjustment_dtl.id_service_center = GL_payment_deposit_Due_Date_For_AR.id_service_center LEFT OUTER JOIN " & _
        '                                " cust_company ON cust_trtab_main.cod_company = cust_company.cod_company LEFT OUTER JOIN " & _
        '                                " bas_zipcode ON cust_trtab_main.zip = bas_zipcode.zipcode" & _
        '                                " WHERE(GL_account_adjustment_head.amount_adjustment < 0) and GL_account_adjustment_dtl.amount_adjustment <> 0  and GL_account_adjustment_dtl.id_service_center in " & txtinvoicebalance & " AND " & Strwhere

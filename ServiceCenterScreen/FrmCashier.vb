...
Public Class FrmCashier
    ...
    Public ChargeFlagVar As Boolean = False
    Public CheckFlagVar As Boolean = False
    Public CreditHoldFlagVar As Boolean = False
    Public IsReturnFlagVar As Boolean = False
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.Panel3.Controls.Add(Me.CustomerCod1)
        ...
        Me.CustomerCod1.Location = New System.Drawing.Point(94, 28)
        ...
        Me.CustomerCod1.Size = New System.Drawing.Size(94, 20)
        Me.CustomerCod1.TabIndex = 228
        Me.CustomerCod1.Visible = False
        '
        'TxtAuthNo
        '
...
#End Region
    ...
    Private Sub EnableDisableBtn()
        Select Case ThisFormStatus
            Case MainModule.WorkStates.AddNew, MainModule.WorkStates.Edit
                ...
                ChkCheck.Enabled = CheckFlagVar
                ChkCreditCard.Enabled = True
                ChkAR.Enabled = ChargeFlagVar And Not CreditHoldFlagVar
                ChkOverPayment.Enabled = IIf(NumOverPaymentBalance.Numbervalue > 0, True, False)
                ChkDownPayment.Enabled = IIf(NumDownPaymentBalance.Numbervalue > 0, True, False)
                'NumARAmount.Enabled = True
                'TXTCardNo.Enabled = True
                'NumCashRendered.Enabled = True
                'NumChangeDue.Enabled = True
                'NumChargeAmount.Enabled = True
                'NumCheckAmount.Enabled = True
                'TxtAuthNo.Enabled = True
                'TxtCheckNo.Enabled = True
                'DteDueDate.Enabled = True
                'NumYear.Enabled = True
                'NumMonth.Enabled = True
                'CmbARTerms.Enabled = True
                'CmbCCType.Enabled = True
                BtnPaymentOnHold.Enabled = True
                BtnSaveAndPrint.Enabled = True
                BtnSaveNoPrint.Enabled = True
    ...
    Private Sub LoadCashierInformation()
        ...
        End If
        Try
            CmdGeneral.CommandText = "SELECT isnull(BalanceOverPaymentAmount,0) as BalanceOverPaymentAmount FROM View_Balance_OverPayment_Amount where cod_customer=" & Qt(CustomerCod1.CustomerCod)
            NumOverPaymentBalance.Text = CmdGeneral.ExecuteScalar()
        Catch ex As Exception
            NumOverPaymentBalance.Text = 0
        End Try
        Try
            CmdGeneral.CommandText = "SELECT isnull(BalanceDownPaymentAmount,0) as BalanceDownPaymentAmount FROM View_Balance_DownPayment_Amount where cod_customer=" & Qt(CustomerCod1.CustomerCod)
    ...
    '    End If
    '    CmdGeneralForReader.CommandText = "Select ID_payment From GL_payment_deposit where refer_type='1' AND id_service_center=" & Qt(TxtRef.Text)
    ...
    '        Select Case i
    '            Case 1
    '                AmountVar = NumCashRendered.Text - NumChangeDue.Text
    '            Case 2
    '                AmountVar = NumCheckAmount.Text
    '            Case 3
    '                AmountVar = NumChargeAmount.Text
    '            Case 4
    '                AmountVar = NumARAmount.Text
    '            Case 5
    '                AmountVar = NumOverPayment.Text
    '            Case 6
    '                AmountVar = NumDownPayment.Text
    '        End Select
    '        If AmountVar > 0 Then
    '            Select Case i
    '                Case 1
    '                    CmdGeneral.CommandText = "Insert Into GL_payment_deposit (id_service_center,refrence_no,refer_type,payment_type,payment_amount) values (" & Qt(TxtRef.Text) & ",NULL,'1'," & Qt(Str(i).Trim) & "," & AmountVar & ");Select @@Identity from GL_payment_deposit"
    '                    ID_paymentVar = CmdGeneral.ExecuteScalar()

    '                    CmdGeneral.CommandText = "Insert Into GL_payment_cash (ID_payment,cash_rendered,change_due) values (" & ID_paymentVar & "," & NumCashRendered.Text & "," & NumChangeDue.Text & ")"
    '                    CmdGeneral.ExecuteNonQuery()
    '                Case 2
    '                    CmdGeneral.CommandText = "Insert Into GL_payment_deposit (id_service_center,refrence_no,refer_type,payment_type,payment_amount) values (" & Qt(TxtRef.Text) & ",NULL,'1'," & Qt(Str(i).Trim) & "," & AmountVar & ");Select @@Identity from GL_payment_deposit"
    '                    ID_paymentVar = CmdGeneral.ExecuteScalar()

    '                    CmdGeneral.CommandText = "Insert Into GL_payment_check (ID_payment,check_no,check_Date) values (" & ID_paymentVar & "," & Qt(TxtCheckNo.Text) & ",'')"
    '                    CmdGeneral.ExecuteNonQuery()
    '                Case 3
    '                    CmdGeneral.CommandText = "Insert Into GL_payment_deposit (id_service_center,refrence_no,refer_type,payment_type,payment_amount) values (" & Qt(TxtRef.Text) & ",NULL,'1'," & Qt(Str(i).Trim) & "," & AmountVar & ");Select @@Identity from GL_payment_deposit"
    '                    ID_paymentVar = CmdGeneral.ExecuteScalar()

    '                    CmdGeneral.CommandText = "Insert Into GL_payment_credit (ID_payment,cc_type,cc_card_no,cc_expire_date_year,cc_expire_date_month,cc_auth_no) values (" & ID_paymentVar & "," & Qt(CmbCCType.SelectedValue) & "," & Qt(TXTCardNo.Text) & "," & Qt(NumYear.Text) & "," & Qt(NumMonth.Text) & "," & Qt(TxtAuthNo.Text) & ")"
    '                    CmdGeneral.ExecuteNonQuery()
    '                Case 4
    '                    If CmdGeneralForReader.Connection.State <> ConnectionState.Open Then
    '                        CmdGeneralForReader.Connection.Open()
    '                    End If
    '                    CmdGeneralForReader.CommandText = "Select top 1 * From GL_TAB_AR_TERM where AR_terms=" & Qt(CmbARTerms.SelectedValue)
    '                    SQLReader = CmdGeneralForReader.ExecuteReader
    '                    If SQLReader.HasRows Then
    '                        SQLReader.Read()
    ...
    '                                End If
    '                                CmdGeneral.CommandText = "Insert Into GL_payment_deposit (id_service_center,refrence_no,refer_type,payment_type,payment_amount) values (" & Qt(TxtRef.Text) & ",NULL,'1'," & Qt(Str(i).Trim) & "," & EachPayvar & ");Select @@Identity from GL_payment_deposit"
    '                                ID_paymentVar = CmdGeneral.ExecuteScalar()
    '                                CmdGeneral.CommandText = "Insert Into GL_payment_AR (ID_payment,AR_terms,AR_due_date) values (" & ID_paymentVar & "," & Qt(CmbARTerms.SelectedValue) & "," & Qt(Format(CType(DteDueDate.Text, Date).AddMonths(NO_OF_PAYMENTCounter), PubDateFormat)) & ")"
    '                                CmdGeneral.ExecuteNonQuery()
    '                            Next
    '                        Else
    '                            CmdGeneral.CommandText = "Insert Into GL_payment_deposit (id_service_center,refrence_no,refer_type,payment_type,payment_amount) values (" & Qt(TxtRef.Text) & ",NULL,'1'," & Qt(Str(i).Trim) & "," & AmountVar & ");Select @@Identity from GL_payment_deposit"
    '                            ID_paymentVar = CmdGeneral.ExecuteScalar()
    '                            CmdGeneral.CommandText = "Insert Into GL_payment_AR (ID_payment,AR_terms,AR_due_date) values (" & ID_paymentVar & "," & Qt(CmbARTerms.SelectedValue) & "," & Qt(DteDueDate.Text) & ")"
    '                            CmdGeneral.ExecuteNonQuery()
    '                        End If
    '                    End If
    '                Case 5
    ...
    '                    CmdGeneral.CommandText = "Insert Into GL_payment_deposit (id_service_center,refrence_no,refer_type,payment_type,payment_amount,refrence_no_Gl_receive_payment) values (" & Qt(TxtRef.Text) & ",NULL,'1'," & Qt(Str(i).Trim) & "," & AmountVar & "," & Qt(refrence_no_Gl_receive_paymentVar) & ")"
    ...
    '                    CmdGeneral.CommandText = "Insert Into GL_payment_deposit (id_service_center,refrence_no,refer_type,payment_type,payment_amount,Downpayment_Wo_No) values (" & Qt(TxtRef.Text) & ",NULL,'1'," & Qt(Str(i).Trim) & "," & AmountVar & "," & Qt(Downpayment_Wo_NoVar) & ")"
    '                    ID_paymentVar = CmdGeneral.ExecuteNonQuery
    '            End Select
    '        End If
    '    Next
    ...
    'End Sub
    'Private Sub ConvertToInvoice()
    '    If Mid(TxtRef.Text.Trim, 1, 2) = "WO" Or Mid(TxtRef.Text.Trim, 1, 2) = "ES" Then
    ' chon fekr mikonam kari dart inja nabayad anjam shavad , comment kardam
    '    End If
    'End Sub
    Private Sub BtnPaymentOnHold_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPaymentOnHold.Click
        ...
        IsPrintFlag = False
    End Sub
    Private Function CheckCanSave() As Boolean
        CheckCanSave = True
        If ChkAR.Checked Then
            If CmbARTerms.Text.Trim.Length = 0 Then
                MsgBox("Please Fill A/R Terms")
                CheckCanSave = False
                Exit Function
            End If
            If Mid(DteDueDate.Text, 1, 2).Trim.Length = 0 Then
                MsgBox("Please Fill A/R Due Date")
                CheckCanSave = False
                Exit Function
            End If
        End If
        If ChkCreditCard.Checked Then
            If CmbCCType.Text.Trim.Length = 0 Then
                MsgBox("Please Fill C.C. Type")
                CheckCanSave = False
                Exit Function
            End If
        End If
        If Not (ChkCreditCard.Checked Or ChkAR.Checked Or ChkCash.Checked Or ChkCheck.Checked Or ChkOverPayment.Checked Or ChkDownPayment.Checked) Then
            MsgBox("Please select the kind of Payment")
            CheckCanSave = False
            Exit Function
        End If
        If NumOverPaymentBalance.Numbervalue - NumOverPayment.Numbervalue > 0 And ChkAR.Checked Then
            MsgBox("Please use Over Payment Amount")
            CheckCanSave = False
            Exit Function
        End If
        If NumDownPaymentBalance.Numbervalue - NumDownPayment.Numbervalue > 0 And ChkAR.Checked Then
            MsgBox("Please use Down Payment Amount")
            CheckCanSave = False
            Exit Function
        End If
    End Function
    Private Sub BtnSaveNoPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSaveNoPrint.Click
        If NumBalance.Text = 0 Then
            If Not CheckCanSave() Then
                Exit Sub
            End If
            ...
            Me.Close()
        Else
            MsgBox("The Balance is not Zero")
        End If
    End Sub
    ...
    Private Sub NumCashRendered_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles NumCashRendered.TextChanged
        ...
        End Try
    End Sub
    Private Sub NumCashRendered_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles NumCashRendered.LostFocus
        BtnSaveAndPrint.Focus()
    End Sub
    Private Sub TxtCheckNo_LostFocus(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtCheckNo.LostFocus
        BtnSaveAndPrint.Focus()
    End Sub
    Private Sub TxtAuthNo_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles TxtAuthNo.LostFocus
        BtnSaveAndPrint.Focus()
    End Sub
    ...
    Private Sub ChkCash_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ChkCash.CheckedChanged
        If NumDownPaymentBalance.Numbervalue > 0 And NumDownPayment.Numbervalue = 0 And ChkCash.Checked Then
            MsgBox("This Work Order has Some Down Payment.")
        End If
        Call EnableDisableField()
        If ChkCash.Checked Then
            If ChkMultipleCheck() = 1 Then
                NumCashRendered.Text = NumAmount.Text
            End If
            BtnSaveAndPrint.Focus()
        Else
            NumCashRendered.Text = 0
        End If
        Call ChkMultipleCheck()
    End Sub
    Private Sub ChkCheck_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ChkCheck.CheckedChanged
        If NumDownPaymentBalance.Numbervalue > 0 And NumDownPayment.Numbervalue = 0 And ChkCheck.Checked Then
            MsgBox("This Work Order has Some Down Payment.")
        End If
        Call EnableDisableField()
        If ChkCheck.Checked Then
            If ChkMultipleCheck() = 1 Then
                NumCheckAmount.Text = NumAmount.Text
            End If
            TxtCheckNo.Focus()
        Else
            NumCheckAmount.Text = 0
        ...
        Call ChkMultipleCheck()
    End Sub
    Private Sub ChkCreditCard_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ChkCreditCard.CheckedChanged
        If NumDownPaymentBalance.Numbervalue > 0 And NumDownPayment.Numbervalue = 0 And ChkCreditCard.Checked Then
            MsgBox("This Work Order has Some Down Payment.")
        End If
        Call EnableDisableField()
        If ChkCreditCard.Checked Then
            If ChkMultipleCheck() = 1 Then
                NumChargeAmount.Text = NumAmount.Text
            End If
            CmbCCType.Focus()
        Else
            NumChargeAmount.Text = 0
    ...
    Private Sub ChkAR_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ChkAR.CheckedChanged
        If NumDownPaymentBalance.Numbervalue > 0 And NumDownPayment.Numbervalue = 0 And ChkAR.Checked Then
            MsgBox("This Work Order has Some Down Payment.")
        End If
        Call EnableDisableField()
        If ChkAR.Checked Then
            BtnDetail.Enabled = True
            Call CmbARTerms_SelectedIndexChanged(sender, e)
            If ChkMultipleCheck() = 1 Then
                NumARAmount.Text = NumAmount.Text
            End If
            BtnSaveAndPrint.Focus()
            CmbARTerms.SelectedValue = CustomerCod1.AR_term & ""
        Else
            BtnDetail.Enabled = False
            NumARAmount.Text = 0
            DteDueDate.Text = Format(Now.Today, PubDateFormat)
        End If
        Call ChkMultipleCheck()
    End Sub
    Private Sub ChkOverPayment_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ChkOverPayment.CheckedChanged
        Call EnableDisableField()
        If ChkOverPayment.Checked Then
            If ChkMultipleCheck() = 1 Then
                NumOverPayment.Text = IIf(NumAmount.Numbervalue > NumOverPaymentBalance.Numbervalue, NumOverPaymentBalance.Text, NumAmount.Text)
            End If
            BtnSaveAndPrint.Focus()
        Else
            NumOverPayment.Text = 0
    ...
    Private Sub ChkDownPayment_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ChkDownPayment.CheckedChanged
        If CmdGeneral.Connection.State <> ConnectionState.Open Then
            CmdGeneral.Connection.Open()
        End If
        Select Case Mid(TxtRef.Text, 1, 2)
            Case "WO"
                CmdGeneral.CommandText = "SELECT Count(*) FROM GL_Paid_received WHERE id_service_center=" & Qt(TxtRef.Text) & " AND tran_type=4"
                If CmdGeneral.ExecuteScalar = 0 Then
                    MsgBox("This Down Payment Belong To another Work Order")
                End If
            Case "IN"
                ...
                If TempVar.Trim.Length > 0 Then
                    CmdGeneral.CommandText = "SELECT Count(*) FROM GL_Paid_received WHERE id_service_center=" & Qt(TempVar) & " AND tran_type=4"
                    If CmdGeneral.ExecuteScalar > 0 Then
                        MsgBox("This Down Payment Belong To another Work Order")
                    End If
                End If
        ...
        If ChkDownPayment.Checked Then
            If ChkMultipleCheck() = 1 Then
                NumDownPayment.Text = IIf(NumAmount.Numbervalue > NumDownPaymentBalance.Numbervalue, NumDownPaymentBalance.Text, NumAmount.Text)
            End If
            BtnSaveAndPrint.Focus()
        Else
            NumDownPayment.Text = 0
        ...
        Call ChkMultipleCheck()
    End Sub
    Private Function ChkMultipleCheck() As Integer
        Dim CheckedCounter As Integer = 0
        If ThisFormStatus = MainModule.WorkStates.AddNew Or ThisFormStatus = MainModule.WorkStates.Edit Then
            ...
            End If
        End If
        ChkMultipleCheck = CheckedCounter
    End Function
    Private Sub CmbARTerms_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmbARTerms.SelectedIndexChanged
        If ThisFormStatus = MainModule.WorkStates.AddNew Or ThisFormStatus = MainModule.WorkStates.Edit Then
            DteDueDate.Text = Format(Date.Today, PubDateFormat)
            ...
            CmdGeneralForReader.CommandText = "Select top 1 * From GL_TAB_AR_TERM where AR_terms=" & Qt(CmbARTerms.SelectedValue)
            SQLReader = CmdGeneralForReader.ExecuteReader
            If SQLReader.HasRows And Trim(CmbARTerms.SelectedValue).Length > 0 Then
                SQLReader.Read()
                Select Case SQLReader.Item("KIND_AR")
                    Case 1
                        DteDueDate.Text = Format(CType(DteDueDate.Text, Date).AddDays(SQLReader.Item("ST_NET_DUE_IN")), PubDateFormat)
                    Case 2
                        Dim BeforDatevar As String = DteDueDate.Text
                        If Val(Mid(DteDueDate.Text, 4, 2)) >= SQLReader.Item("DD_NET_DUE_BEFORE") Then
                            DteDueDate.Text = Format(CType(DteDueDate.Text, Date).AddMonths(1), PubDateFormat)
                        End If
                        DteDueDate.Text = Mid(DteDueDate.Text, 1, 3) & Getcod(Str(SQLReader.Item("DD_NET_DUE_BEFORE")), 2) & Mid(DteDueDate.Text, 6, 5)
                        If CType(DteDueDate.Text, Date).Subtract(CType(BeforDatevar, Date)).TotalDays < SQLReader.Item("DD_IF_ISSUED_WITHIN_DAYS") Then
                            DteDueDate.Text = Format(CType(DteDueDate.Text, Date).AddMonths(1), PubDateFormat)
                        End If
                    Case 3
                        DteDueDate.Text = Mid(DteDueDate.Text, 1, 3) & Getcod(Str(SQLReader.Item("MP_BEFORE_DAY")), 2) & Mid(DteDueDate.Text, 6, 5)
                End Select
            Else
                Select Case CustomerCod1.KIND_AR
                    Case 1
                        DteDueDate.Text = Format(CType(DteDueDate.Text, Date).AddDays(CustomerCod1.ST_NET_DUE_IN), PubDateFormat)
                    Case 2
                        Dim BeforDatevar As String = DteDueDate.Text
                        If Val(Mid(DteDueDate.Text, 4, 2)) >= CustomerCod1.DD_NET_DUE_BEFORE Then
                            DteDueDate.Text = Format(CType(DteDueDate.Text, Date).AddMonths(1), PubDateFormat)
                        End If
                        DteDueDate.Text = Mid(DteDueDate.Text, 1, 3) & Getcod(Str(CustomerCod1.DD_NET_DUE_BEFORE), 2) & Mid(DteDueDate.Text, 6, 5)
                        If CType(DteDueDate.Text, Date).Subtract(CType(BeforDatevar, Date)).TotalDays < CustomerCod1.DD_IF_ISSUED_WITHIN_DAYS Then
                            DteDueDate.Text = Format(CType(DteDueDate.Text, Date).AddMonths(1), PubDateFormat)
                        End If
                    Case 3
                        DteDueDate.Text = Mid(DteDueDate.Text, 1, 3) & Getcod(Str(CustomerCod1.MP_BEFORE_DAY), 2) & Mid(DteDueDate.Text, 6, 5)
                End Select
            End If
            SQLReader.Close()
        End If
    ...
    End Sub
    Private Sub NumOverPayment_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles NumOverPayment.Leave
        If NumOverPayment.Numbervalue > NumOverPaymentBalance.Numbervalue And NumOverPayment.Enabled Then
            NumOverPayment.Text = NumOverPaymentBalance.Text
        End If
    End Sub
    Private Sub NumDOWNPayment_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles NumDownPayment.Leave
        If NumDownPayment.Numbervalue > NumDownPaymentBalance.Numbervalue And NumDownPayment.Enabled Then
            NumDownPayment.Text = NumDownPaymentBalance.Text
        End If
    End Sub
End Class
